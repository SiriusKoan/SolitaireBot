        ...
        if chat_id in games:
            if '@' + query.from_user.username == games[chat_id].get_current_player():
                ...
                if query.query[0] == start_with:
                    ...
                    warning = 'The word has to start with "%s"'%start_with
                    raise Exception
            else:
                warning = 'You are not the current player.'
                raise Exception
        else:
            warning = 'You have to start a game first.'
            raise Exception
    except Exception:
        response = [types.InlineQueryResultArticle(0, warning, types.InputTextMessageContent(warning))]
        bot.answer_inline_query(query.id, response, cache_time = 1)
